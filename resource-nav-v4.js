(function () {
  ...
  const RESOURCE_DATA_VERSION = "20260713-resource-nav-v4";
  ...
  function parseParams() {
    ...
    renderIdentity("checking", source === "web" ? "OB 網站入口，等待身份確認" : "未連結 OB 網站，請回 OB 網站重新開啟入口");
  }
  ...
  function rememberIdentity(user) {
    ...
    const payload = {
      id: String(user.id || ""),
      name: String(user.name || "OB 網站 使用者"),
      savedAt: nowIso(),
    };
    try {
  ...
  function readRememberedIdentity() {
    try {
      ...
      return {
        id: String(data.id || ""),
        name: String(data.name || "OB 網站 使用者"),
  ...
  function identityText() {
    ...
    const name = state.sessionUser.name || "OB 網站 使用者";
  ...
  function renderIdentity(status, reason) {
    ...
    if (status === "linked") {
      ...
      pill.textContent = "已連結 OB 網站：" + identityText();
      updateRetrySessionButton();
      return;
    }
    if (status === "offline") {
    ...
    if (status === "error") {
      ...
      const cached = rememberedIdentityText();
      if (cached) {
        pill.textContent = "曾連結 OB 網站：" + cached + "；" + (reason || "目前 API/Session 無法驗證");
        updateRetrySessionButton();
        return;
      }
    }
    pill.textContent = reason || "OB 網站 身份未確認";
    updateRetrySessionButton();
  }

  function sessionReasonLabel(reason) {
    const labels = {
      verified: "已完成 OB 網站 身份驗證。",
      no_session: "網址沒有 session；請從 OB 網站 資源導航入口重新開啟。",
      ...
      session_expired: "後端回覆 session 無效或過期；token 已不存在或已超過有效時間，請回 OB 網站 重新點入口。",
      api_unreachable: "公開 API 網址連不上；通常是 Cloudflare tunnel 已失效、DNS 解析不到，或 網站 API 沒有啟動。",
      ...
      missing_session: "缺少 OB 網站 session 或 API 網址；已改用本機預覽結果。",
    };
  ...
  function renderSessionDebug() {
    ...
    verifyStatus.textContent = state.sessionValid ? "已驗證 OB 網站 身份" : "未完成後端驗證";
  ...
  async function verifySession() {
    ...
    if (!state.sessionToken) {
      ...
      loginStatus.textContent = state.source === "web"
        ? "已從 OB 網站 入口開啟，但網址沒有 session。可先看本機快取與本機預覽；若要保存到後端，請回 OB 網站 重新點入口。"
        : "未取得 OB 網站 session：可瀏覽與點選資源，也可先產生本機預覽結果；若要保存到後端，請回 OB 網站 重新點入口。";
    ...
    if (!state.apiBase) {
      ...
      loginStatus.textContent = "已取得 session，但網址沒有 api_base，網站不知道要向哪個 網站 API 驗證身份。可先看本機快取與本機預覽；若要保存到後端，請確認 Bot 入口已更新後重新開啟。";
    ...
    try {
      ...
      state.canManageResources = Boolean(data.can_manage_resources);
      rememberSessionEntryUrl({ verified: true });
      rememberIdentity(state.sessionUser);
      renderIdentity("linked");
      loginStatus.textContent = "已連結 OB 網站：" + identityText() + "。草稿與結果會保存到你的資源組合工作台。";
      renderSessionDebug();
    } catch (error) {
      if (await applyRuntimeApiBase()) {
        return verifySession();
      ...
      state.sessionValid = false;
      state.canManageResources = false;
      ...
      loginStatus.textContent = "已從 OB 網站 入口開啟，但後端驗證未通過：" + sessionReasonLabel(state.sessionFailureReason) + " 可先看本機快取與本機預覽；若要保存到後端，請回 OB 網站 重新點入口。";
      renderSessionDebug();
  ...
  async function retrySessionVerification() {
    ...
    state.sessionFailureReason = "";
    renderIdentity("checking", "正在重新驗證 OB 網站 身份");
    try {
      await verifySession();
  ...
  async function loadResourceCatalog() {
    ...
    const suffix = state.canManageResources ? "?include_offline=1" : "";
    const data = await apiFetch("/api/v1/resource/catalog" + suffix, { method: "GET", headers: {} });
    state.resources = Array.isArray(data.resources) ? data.resources : [];
    state.canManageResources = Boolean(data.can_manage_resources);
    if (!state.canManageResources) state.catalogView = "online";
    renderCatalogControls();
    return state.resources;
  }

  function renderCatalogControls() {
    const control = $("adminCatalogControl");
    if (!control) return;
    control.hidden = !state.canManageResources;
    const online = $("showOnlineResources");
    const offline = $("showOfflineResources");
    online.classList.toggle("is-active", state.catalogView === "online");
    offline.classList.toggle("is-active", state.catalogView === "offline");
    online.setAttribute("aria-pressed", String(state.catalogView === "online"));
    offline.setAttribute("aria-pressed", String(state.catalogView === "offline"));
  }

  async function changePublication(resource, nextStatus) {
    if (!state.canManageResources || !state.sessionValid || state.catalogBusyIds.has(resource.id)) return;
    const actionLabel = nextStatus === "offline" ? "下線" : "恢復上線";
    if (!window.confirm("確定要「" + actionLabel + "」資源：\n" + resource.name + "？\n\n此操作不會刪除卡片或歷史資源包。")) return;
    const originalIndex = state.resources.findIndex((item) => item.id === resource.id);
    state.catalogBusyIds.add(resource.id);
    if (originalIndex >= 0) state.resources.splice(originalIndex, 1);
    renderCards();
    const notice = $("navPackageNotice");
    if (notice) notice.textContent = "正在" + actionLabel + "：" + resource.name + "…";
    try {
      await apiFetch("/api/v1/resource/cards/" + encodeURIComponent(resource.id) + "/publication", {
        method: "PATCH",
        body: JSON.stringify({
      ...
      await loadResourceCatalog();
      if (notice) notice.textContent = "已" + actionLabel + "：" + resource.name + "。歷史資源包不受影響。";
    } catch (error) {
      if (originalIndex >= 0) state.resources.splice(originalIndex, 0, resource);
      else state.resources.push(resource);
      if (notice) notice.textContent = actionLabel + "失敗，卡片已恢復顯示：" + resource.name + "。";
      console.info("resource publication update failed", error);
    } finally {
      state.catalogBusyIds.delete(resource.id);
      renderCards();
    }
  }
  ...
  function sourceTrustLabel(resource) {
    const text = [resource.source_type, resource.confidence].filter(Boolean).join(" ").toLowerCase();
    if (text.includes("official") || text.includes("government") || text.includes("官方") || text.includes("a級")) {
      return "官方來源";
    }
    if (text.includes("c級") || text.includes("線索") || text.includes("lead_only")) return "查證線索";
    return "機構官網";
  }

  function reviewDateOverdue(value) {
    const text = String(value || "").trim();
  ...
  function verificationPresentation(resource) {
    const evidence = String(resource.verification_evidence_status || "");
    const recency = String(resource.verification_recency_status || "");
    const availability = String(resource.verification_availability_status || "");
    const overdue = reviewDateOverdue(resource.next_review_at);
    let label = resource.last_checked_at ? "已記錄來源檢查" : "待建立查證紀錄";
    let tone = "neutral";
    if (availability === "retired_with_evidence") {
      label = "來源已停用";
      tone = "danger";
    } else if (["unknown", "unreachable_retry"].includes(availability) && evidence) {
      label = "網址待重試";
      tone = "neutral";
      ...
      label = "待複查";
      tone = "warning";
    } else if (evidence === "partial" || availability === "needs_contact" || ["changed", "stale"].includes(recency)) {
      label = "部分證據";
      tone = "warning";
    } else if (evidence === "complete" && recency === "current" && availability === "verified_online") {
      label = "已遠端查證";
      tone = "success";
    }
    const needsContact = availability === "needs_contact" || evidence !== "complete" || !evidence;
    return { label, tone, needsContact };
  }

  function verificationMethodText(resource) {
    const checked = resource.last_checked_at || "尚未完成";
    return "最近查證：" + checked +
      "。本工具以公開網站確認來源頁、資源名稱及網頁可取得的電話、地址與服務文字；" +
      "目前名額、實際收案、費用、人力及服務品質仍需直接聯繫確認。";
  }
  ...
  async function exportPackage(item, format, button) {
    if (!state.sessionValid) {
      window.alert("請從 OB 網站 重新開啟入口後再匯出封包。");
      return;
    }
    const oldText = button.textContent;
  ...
  async function importResourcePackPayload(payload, button) {
    if (!state.sessionValid) {
      $("importResourcePackStatus").textContent = "請從 OB 網站 重新開啟入口後再匯入到個人工作台。";
      return;
    }
    ...
    try {
      ...
      return parsed
        ...
        .map((item) => ({
          ...
          outputMode: String(item.outputMode || "full") === "family" ? "full" : String(item.outputMode || "full"),
  ...
  function syncPackageFromState() {
    ...
    item.outputMode = item.outputMode === "family" ? "full" : (item.outputMode || "full");
  ...
  function renderWorkbench() {
    ...
    packages.forEach((item) => {
      ...
      remove.title = state.sessionValid ? "" : "重新連結 OB 網站 後才能刪除後端資源組合。";
  ...
  function renderExchange() {
    ...
    list.innerHTML = "";
    if (!state.sessionValid) {
      status.textContent = "未連結 OB 網站，請回 OB 網站 重新開啟入口後再匯入或匯出資源副本。";
      empty.hidden = false;
  ...
  function normalizeServerPackage(record) {
    return {
      ...
      outputMode: String(record.output_mode || "full") === "family" ? "full" : String(record.output_mode || "full"),
  ...
  function packagePayload(item, overrides) {
    return {
      ...
      outputMode: item.outputMode === "family" ? "full" : (item.outputMode || "full"),
  ...
  function setupFilters() {
    ...
    $("showOnlineResources").addEventListener("click", () => {
      state.catalogView = "online";
      clearSmartSearchResults();
      renderCatalogControls();
      renderCards();
    });
    $("showOfflineResources").addEventListener("click", () => {
      state.catalogView = "offline";
      clearSmartSearchResults();
      renderCatalogControls();
      renderCards();
    });
    ...
    $("clearImportResourcePackButton").addEventListener("click", () => {
      ...
      $("importResourcePackStatus").textContent = "匯入會歸到目前 OB 網站 使用者底下。";
    });
  }

  function matchesBaseResource(resource) {
    const publicationStatus = resource.publication_status || "online";
    if (state.catalogView === "offline") {
      if (!state.canManageResources || publicationStatus !== "offline") return false;
    } else if (publicationStatus !== "online") {
      return false;
    }
  ...
  async function requestSmartSearch() {
    if (!state.sessionValid) throw new Error("auth_required");
    const data = await apiFetch("/api/v1/resource/smart-search", {
      method: "POST",
      body: JSON.stringify({
  ...
  function renderCards() {
    ...
    scoredResults.forEach(({ resource, score }) => {
      ...
      const publicationAction = node.querySelector(".publication-action");
      if (state.canManageResources) {
        const isOffline = (resource.publication_status || "online") === "offline";
        publicationAction.hidden = false;
        publicationAction.textContent = isOffline ? "恢復上線" : "×";
        publicationAction.classList.toggle("is-restore", isOffline);
        publicationAction.setAttribute("aria-label", (isOffline ? "恢復上線：" : "下線：") + resource.name);
        publicationAction.title = isOffline ? "恢復上線" : "下線（不刪除）";
        publicationAction.disabled = state.catalogBusyIds.has(resource.id);
        publicationAction.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          changePublication(resource, isOffline ? "online" : "offline");
        });
      }
      ...
      node.querySelector(".confidence").textContent = sourceTrustLabel(resource);
      const verification = verificationPresentation(resource);
      const verificationStatus = node.querySelector(".verification-status");
      verificationStatus.textContent = verification.label;
      verificationStatus.classList.add("is-" + verification.tone);
      const verificationNote = node.querySelector(".verification-note");
      verificationNote.hidden = !verification.needsContact;
      verificationNote.textContent = "服務現況需聯繫確認";
      node.querySelector(".trust-summary").textContent = [
        sourceTrustLabel(resource),
        verification.label,
        "查證 " + (resource.last_checked_at || "待確認"),
      ].join(" · ");
      ...
      node.querySelector(".verification-method").textContent = verificationMethodText(resource);
      const verificationLink = node.querySelector(".verification-source-link");
      if (resource.source_url) {
        verificationLink.href = resource.source_url;
      } else {
      ...
      const sourceAction = node.querySelector(".source-action");
      if (resource.source_url) {
        sourceAction.href = resource.source_url;
        sourceAction.addEventListener("click", (event) => {
          event.stopPropagation();
          if (!confirmOpenSource(resource, resource.source_url)) event.preventDefault();
        });
      } else {
        sourceAction.hidden = true;
      }
      const toggleButton = node.querySelector(".toggle-resource-action");
      const selected = state.packageIds.has(resource.id);
      toggleButton.textContent = selected ? "移出組合" : "加入組合";
  ...
  function buildPackageOutput(items, mode) {
    ...
    return buildHandoffPackageText(items);
  }
  ...
  async function openOrCreatePackageResult(item, button, options = {}) {
    if (item.status === "result_ready") {
      if (item.shareUrl) openResultUrl(item.shareUrl, Boolean(options.print));
      else openLocalResult("private_web_result", options);
      return;
    }
  ...
  async function submitResourcePackage(sourceButton, options = {}) {
    ...
    $("packageStatus").textContent = "正在建立私密資源包結果；有可用的分享連結時會一併顯示。";
    try {
      ...
      if (data.share_status === "pending") {
        $("packageStatus").textContent = "私密結果已建立，分享連結正在背景確認；先開啟網站結果預覽。";
  ...
  async function updateCurrentDraft() {
    ...
    try {
      ...
      if (state.sessionValid && state.packageIds.size) {
        ...
        $("packageStatus").textContent = "已更新本機暫存。未連結 OB 網站，不能保存到個人工作台。";
      }
      renderWorkbench();
    } catch (error) {
  ...
  async function init() {
    ...
    setupFilters();
    try {
      const topicsData = await loadJson(
        versionedAsset("./resource-nav-topics.json"),
        versionedAsset("../../../data/resource_nav/topics.json"),
      );
