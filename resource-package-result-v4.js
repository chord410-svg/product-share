(function () {
  ...
  function currentResultMode() {
    const mode = new URLSearchParams(window.location.search).get("output") || localStorage.getItem("resource_nav_result_output_mode_v1") || "full";
    return mode === "family" ? "full" : mode;
  }
  ...
  function renderResult() {
    ...
    if (isLocalPreviewMode() && localPreviewReason() === "publish_pending") {
      ...
      if (body) {
        body.textContent = "主結果已先在此瀏覽器顯示；登入保護的正式結果與 QR Code 正在背景完成。稍後回到「我的資源組合」即可取得正式連結與 QR。";
      }
    }
    ...
    const legacyResultLink = $("discordResultLink");
    legacyResultLink.hidden = true;
    legacyResultLink.removeAttribute("href");
    legacyResultLink.setAttribute("aria-hidden", "true");
    legacyResultLink.tabIndex = -1;
